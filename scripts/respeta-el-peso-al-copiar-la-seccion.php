 ...
 * portada. El guion que rehizo la portada de marcas copiaba los pesos del molde
 * y aun asi la pantalla salio desordenada, y hasta que no se puso el molde y la
 ...
 * en cuatro enlaces. Asi que la receta se prueba aqui, con el molde de colores
...
$molde = \Drupal::entityTypeManager()->getStorage('node')->load(9);
if (!$molde) {
  echo "\n No esta el nodo 9, que es el molde. Sin el no hay nada que copiar.\n\n";
  return;
}
...
$pesosMolde = $pesos($seccionMolde);
foreach ($pesosMolde as $id => $peso) {
  printf("   peso %-3s %s\n", $peso, $id);
}

/**
 * Las piezas del molde recien copiadas, con su peso puesto.
 *
 ...
 */
$copiarPiezas = static function (Section $seccionMolde): array {
  $piezas = [];
...
$mal = new Section($seccionMolde->getLayoutId(), $seccionMolde->getLayoutSettings());
foreach ($copiarPiezas($seccionMolde) as $pieza) {
...
$bien = new Section($seccionMolde->getLayoutId(), $seccionMolde->getLayoutSettings(), $copiarPiezas($seccionMolde));
...
printf(
  "   appendComponent respeta el molde: %s\n",
  array_keys($pesosMal) === array_keys($pesosMolde) ? 'si' : 'NO, y por eso salio desordenada'
);
printf(
  "   el constructor respeta el molde:  %s\n",
  array_keys($pesosBien) === array_keys($pesosMolde) && $pesosBien === $pesosMolde ? 'si' : 'NO, mirar esto antes de fiarse'
);
